...
allProduct.forEach((product) => {
    ...
    productElem.innerHTML = `<h3 class="product-name">${product}</h3> <div class="review-container"></div>`;
    productsList.append(productElem);
    productElem.classList.add('review-list__item');
    const revieContainer = productElem.querySelector('.review-container');
    const reviews = JSON.parse(localStorage.getItem(product)) || [];

    reviews.forEach((review) => {
        const reviewElem = document.createElement('p');
        reviewElem.classList.add('review-item');
        reviewElem.innerHTML = `${review} <button class="review-del-btn">Удалить</button>`;
        revieContainer.append(reviewElem);
    });
});

const productItemLists = document.querySelectorAll('.product-name');
